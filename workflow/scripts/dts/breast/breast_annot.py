...
rna.index = [b + '_' + i.split('-')[0].split('_')[-1] for i, b in zip(rna.index, rna['Pool'])]
rna = rna.rename(columns={'cell_type': 'celltype', 'Pool': 'batch'})
rna = rna[['celltype', 'batch', 'donor_id']]
