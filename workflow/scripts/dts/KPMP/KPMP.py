...
ID = rna.obs['experiment'].str.split('_').str[1] 
rna.obs_names = [b + '_' + i.split('-')[0].split('_')[-1] for i, b in zip(rna.obs.index, ID)]
rna = rna[rna.obs_names.isin(obs.index), :].copy()
rna.obs = rna.obs[['subclass.l1']]
## First need to map chromosome location to gene names
#rna.var['gene_name-new'] = pd.Series(geneids).reindex(rna.var_names)
