...
ray_tmp_dir = os.path.abspath('/tmp/')

# SCENIC+ paths
scenicplus_tmp_dir = os.path.abspath('tmp2/')
output_filename = os.path.join(scenicplus_tmp_dir, 'scenicplus.h5mu')
# cisTopic paths
cis_topic_tmp_dir = os.path.join(scenicplus_tmp_dir, 'cisTopic')
...
os.makedirs(scenicplus_tmp_dir, exist_ok=True)
...
for batch_id in batch_ids:
    ...
    cell_data.loc[cell_data['batch'] == batch_id, 'barcode'] = \
        cell_data.loc[cell_data['batch'] == batch_id, 'barcode'].str.replace(batch_id + '_', '')
...
chromsizes['Chromosome'] = [chromsizes['Chromosome'][x].split('_')[1] if len(chromsizes['Chromosome'][x].split('_')) > 1
                            else chromsizes['Chromosome'][x] for x in range(len(chromsizes['Chromosome']))]
chromsizes=pr.PyRanges(chromsizes)

bw_paths, bed_paths = export_pseudobulk(
    input_data=cell_data.iloc[:100,:],
...
def qc(
    ...
    if isinstance(regions_bed_filename, pl.DataFrame):
        regions_bed_df_pl = regions_bed_filename
    else:
