...
working_dir = os.path.dirname(mudata)
pre = os.path.basename(peak_file).split('.')[0]
p2g = os.path.basename(peak_file).split('.')[1]
tmp_path = f"{pre}.{p2g}.dictys_tmp"
os.makedir(tmp_path)
...
valid_peak_fname = os.path.join(working_dir, tmp_path, "valid_peak.bed")
...
for count, c in enumerate(clus):
    ...
    atac_fname = os.path.join(working_dir, tmp_path, f"name_clus_{count}.txt")
    with open(atac_fname, "w") as f:
        for n in cell_name:
            f.write(f"{n}\n")

    # Generate pseudo-bam file from fragments belong to the cell cluster
    bam_name = os.path.join(working_dir, tmp_path, f"clus_{count}.bam")
    bai_name = os.path.join(working_dir, tmp_path, f"clus_{count}.bai")
    ...
    footprint_name = os.path.join(working_dir, tmp_path, f"clus_{count}_footprints.tsv.gz")
    homer_name = os.path.join(working_dir, tmp_path, f"clus_{count}_homer.tsv.gz")
    wellington_name = os.path.join(working_dir, tmp_path, f"clus_{count}_wellington.tsv.gz")
    motif_name = os.path.join(working_dir, tmp_path, f"clus_{count}_motifs.tsv.gz")
    binding_name = os.path.join(working_dir, tmp_path, f"clus_{count}_binding.tsv.gz")
...
for count, c in enumerate(clus):
    ...
    footprint_tf_name = os.path.join(working_dir, tmp_path, f"clus_{count}_footprint_to_TF.bed")
    df[['chr', 'srt', 'end', 'TF', 'score']].to_csv(footprint_tf_name, sep='\t', header=False, index=False)
    valid_peak_to_tf_name = os.path.join(working_dir, tmp_path, f"valid_peak_to_clus_{count}_TF.bed")
...
overall_peak_to_TF = pd.concat([
    pd.read_csv(os.path.join(working_dir, tmp_path, f"valid_peak_to_clus_{count}_TF.bed"), sep='\t', header=None)
    for count, c in enumerate(clus)
], axis=0)
